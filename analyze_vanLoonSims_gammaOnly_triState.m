...
startTime = 250;
paramList = {'gamma'}; % {'gamma' 'I' 'sigma' 'alpha'};
inName = ['run_vanLoonSims_' char(codeAll) '_gammaOnly_triState'];
...
inFileList = dir(fullfile('simRes',[inName '_*']));
for curIter = 1:size(inFileList,1)
    tic
    inFile = fullfile(inFileList(curIter).folder,inFileList(curIter).name);
    load(inFile,'p','pR','times','Var')
%     load(fullfile('simRes',[inName '_' code '_' num2str(curIter)]),'p','pR','times','Var')
    for param = paramList
        param_i = find(ismember(paramList,param));
        display(['Iter ' num2str(curIter)])
        display(['Param ' char(param)])
        
        for i = 1:length(pR.(char(param)))
            if curIter==1 %&& strcmp(char(param),'sigma') && i>=3
                end_time = times.(char(param)){i}(end);
                f{i,param_i,curIter} = plot_vanLoonSims(end_time,startTime,times.(char(param)){i},Var.(char(param)){i},pR.(char(param))(i),p.I);
%                 plot_vanLoonSims(900,1,times.(char(param)){i},Var.(char(param)){i},pR.(char(param))(i),p.I)
            end
...
save(fullfile('simRes',[mfilename '_simRes_' code '_compiled']),'durAll','p','pR','code')
end
...
if 1
%% Analyze durations
clearvars -except code paramList
load(fullfile('simRes',[mfilename '_simRes_' code '_compiled']),'durAll','p','pR','code')
...
save(fullfile('simRes',[mfilename '_simRes_' code '_processed']),'dur','durMean','durMedian','p','pR','code')
end
...
% close all
clearvars -except code paramList
load(fullfile('simRes',[mfilename '_simRes_' code '_processed']),'durMean','durMedian','p','pR','code')
...
%     spaceplots
    ...
    figure('WindowStyle','docked'); hold on
    %X1
    X1mean = mean([durMean.(char(param)).X1; durMean.(char(param)).X2],1);
    X1std  = std([durMean.(char(param)).X1; durMean.(char(param)).X2],[],1);
